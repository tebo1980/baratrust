...
export default function DellaAgentPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/agent-demo", {
        ...
        body: JSON.stringify({ agent: "della", input: userMsg }),
      });
      ...
      <main className="flex-1 flex flex-col relative">
        <header className="p-6 border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-400">Lead Screener</p>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500">
              <p>Initialize protocol with Della Engine...</p>
            </div>
          ) : (
        ...
        <div className="p-6 bg-gray-900 border-t border-gray-800">
          <form onSubmit={handleSendMessage} className="flex gap-4">
            <input
              ...
              placeholder="Send a directive to Della..."
